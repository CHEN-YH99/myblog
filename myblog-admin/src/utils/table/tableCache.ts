...
export class TableCache<T> {
  ...
  private generateKey(params: unknown): string {
    ...
    try {
      ...
      // 如果JSON序列化失败（比如循环引用），使用简化的键生成策略
      this.log('JSON序列化失败，使用简化键生成策略:', error)
      return this.generateSimpleKey(params)
    }
  }

  // 简化的键生成策略，避免循环引用问题
  private generateSimpleKey(params: unknown): string {
    if (!params || typeof params !== 'object') {
    ...
    for (const [key, value] of Object.entries(obj)) {
      if (value === null || value === undefined) {
        ...
        // 对于数组，只取长度和第一个元素（如果是基本类型）
        const firstItem = value.length > 0 && (typeof value[0] === 'string' || typeof value[0] === 'number') ? value[0] : 'complex'
        keyParts.push(`${key}:[${value.length}]${firstItem}`)
      } else {
        // 对于复杂对象，只使用类型信息
        keyParts.push(`${key}:object`)
      }
    }
  ...
  // 递归排序对象键（带循环引用检测和深度限制）
  private sortObjectKeys(obj: Record<string, unknown>, visited = new WeakSet(), depth = 0): Record<string, unknown> {
    // 防止无限递归：检查深度限制
    if (depth > 10) {
      return obj
    }
    ...
    for (const key of keys) {
      ...
      // 跳过Vue响应式对象的内部属性
      if (key.startsWith('__') || key === '_isRef' || key === '_isReactive' || key === 'dep') {
        continue
      }
      
      if (value && typeof value === 'object' && !Array.isArray(value) && value.constructor === Object) {
        // 只对纯对象进行递归排序，避免处理 Vue 响应式对象、DOM 元素等
        result[key] = this.sortObjectKeys(value as Record<string, unknown>, visited, depth + 1)
      } else {
        result[key] = value
      }
    }
