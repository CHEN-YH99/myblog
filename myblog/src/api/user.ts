...
export const getUserListFromAdmin = (params?: {
  ...
  const cacheKey = getCacheKey('/api/user/admin-list', params)
...
export const getUserStats = (): Promise<{
  ...
  return withRetry(async () => {
    const [totalRes, activeRes, todayRes, monthRes] = await Promise.all([
      api.get<{ records: any[]; total: number }>({
        url: '/api/users',
      ...
      api.get<{ records: any[]; total: number }>({
        url: '/api/users',
        params: { startTime: todayStart.toISOString(), endTime: now.toISOString(), current: 1, size: 1 },
        showErrorMessage: false
      }),
      api.get<{ records: any[]; total: number }>({
        url: '/api/users',
        params: { startTime: monthStart.toISOString(), endTime: now.toISOString(), current: 1, size: 1 },
        showErrorMessage: false
      })
    ])
 ...
 * @returns 发送结果
 */
export const sendEmailCode = (email: string, type: 'register' | 'reset' | 'change'): Promise<void> => {
  return api.post({
 ...
 * @returns 验证结果
 */
export const verifyEmailCode = (email: string, code: string, type: string): Promise<{
  valid: boolean
  token?: string
}> => {
  return api.post({
